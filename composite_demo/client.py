...
MODEL_PATH_LIST = ['/mnt/workspace/Chatglm3-6b-summary3/', '/mnt/workspace/base']
...
def get_client() -> Client:
    ...
    print(f'{MODEL_PATH},\t{TOKENIZER_PATH}, \t{PT_PATH}')
    return client
...
class HFClient(Client):
    ...
    def stream_chat(
        ...
        **kwargs
        ...
        if input_sequence_length + max_new_tokens >= model.config.seq_length:
            yield "Current input sequence length {} plus max_new_tokens {} is too long. The maximum model sequence length is {}. You may adjust the generation parameter to enable longer chat history.".format(
                input_sequence_length, max_new_tokens, model.config.seq_length
            ), history
            return
    ...
    def generate_stream(
        ...
        text = ''
        for new_text, _ in self.stream_chat(
                self.model,
                self.tokenizer,
                query,
                chat_history,
